...
const Requests = () => {
  ...
  const fetchRequests = async()=>{
    try{
      const res = await axios.get(BASE_URL + "/user/get/request/received",{
        withCredentials:true
      });
...
const { firstName, lastName, photoUrl, age, gender, about } = fromUserId;
  return (
    ...
    <div>
      ...
      <button className="btn btn-soft btn-accent mx-2" onClick={()=> reviewRequest("accepted",request._id)}>Accept</button>
    </div>
    </div>
  )})}
